...
app.get('/api/diag/migrate-db', async (req, res) => {
  try {
    ...
    try {
      await prisma.$queryRaw('ALTER TABLE `Banner` ADD COLUMN `subtitle` VARCHAR(255) NULL');
    } catch (e) {
      console.log("Coluna subtitle talvez já exista:", e.message);
    }
    try {
      await prisma.$queryRaw('ALTER TABLE `Banner` ADD COLUMN `tag` VARCHAR(255) NULL');
    } catch (e) {
