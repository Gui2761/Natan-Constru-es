...
export default function CategoryResults() {
  ...
  const fetchCategoryData = async () => {
    try {
      ...
      const currentCat = c.data.find(cat => cat.slug === slug);
      setCategory(currentCat);
      
      const filtered = p.data.filter(prod => prod.categoryId === currentCat?.id);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto py-10 px-4">
        <div className="flex items-center gap-4 mb-10 border-b border-outline-variant pb-6">
           ...
           <div>
              <p className="text-[10px] font-black text-outline uppercase tracking-widest leading-none mb-1">Departamento</p>
            ...
            {products.map(product => (
              <Card key={product.id} className="p-0 overflow-hidden relative group" hover>
                 ...
                 <div className="aspect-square bg-surface-container overflow-hidden">
                    <img 
                      src={product.images || 'https://images.unsplash.com/photo-1581094288338-2314dddb7ecc?q=80&w=1000'} 
                 ...
                 <div className="p-5">
                    ...
                    <a href={`/produto/${product.id}`} className="block w-full mt-6 bg-primary text-white text-center py-3 rounded-lg uppercase text-xs font-black hover:bg-primary/90 transition-colors">
                       Ver Detalhes
                    </a>
                 </div>
              </Card>
